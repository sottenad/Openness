...
function populateBlogExcerpt(){
	if($('#openBlog').length > 0){
			var monthArray = new Array("January","February","March","April","May","June","July","August","September","October","November","December");
			$.get('RSSProxyOpen.aspx', function(d) {
				$('#openBlog').empty();
				//$('#openBlogNext').empty();
				//alert($(d).text());
				...
				$(d).find('item').each(function(index) {
					if(index<1){
						...
						description = description.replace(/<(?!\s*\/?\s*p|br\b)[^>]*>/gi,''); //strip all tags except paragraph and line break
						var descArr = description.split("\n");
						
						/* Process each line to grab author, then start description at actual blog start */
						for (var i=0, creatorFound=0, start=0, len=descArr.length, description = ''; i<len; ++i) {
							if (descArr[i].match(/Posted by /i) && creatorFound == 0) {
								creator = descArr[i].split(/Posted by /i)[1];
								creatorFound = i+1;
							} else if (descArr[i].match(/\w\w \w\w/) && i >= creatorFound) {
								start = 1;
							}
							
							if (i >= creatorFound && start == 1) {
								description += descArr[i];
							}
						}

						if (creator.length == 0) {
							creator = $(this).find("dc\\:creator").text();
						}
						...
						pubDate = pubDay + ', ' + pubMonth + ' ' + pubDate[1] + ', ' + pubDate[3] + ' by <span class="creator">' + creator + '</span>';
						...
						$(headingLink).attr('target', '_blank');
						
						$(more).attr('href', link);
						$(more).attr('target', '_blank');
						$(more).html('Read Full Article &raquo;');
						$(headingLink).append(title);
						$(heading).append(headingLink);
						$(byline).append(pubDate);
						
						var concatLength = 500;
						...
						$(description).find('a').each(function(index){
							...
							if(tagStart < concatLength && tagEnd > concatLength){	
								concatLength = tagEnd;
							}
						...
						$(entryLink).attr('href', link);
						$(entryLink).attr('target', '_blank');
						$(entryLink).append(span);
						$(span).append(heading);
...
var screen_name=item.from_user||item.user.screen_name;var source=item.source;var user_url="http://"+s.twitter_url+"/"+screen_name;var avatar_size=s.avatar_size;var avatar_url=item.profile_image_url||item.user.profile_image_url;var tweet_url="http://"+s.twitter_url+"/"+screen_name+"/statuses/"+item.id_str;var tweet_time=item.created_at;var tweet_relative_time=relative_time(tweet_time);var tweet_raw_text=item.text;var tweet_text=$([tweet_raw_text]).linkUrl().linkUser().linkHash()[0];var user='<a class="tweet_user" target=\"_blank\" href="'+user_url+'">'+screen_name+'</a>';var join=((s.join_text)?('<span class="tweet_join"> '+join_text+' </span>'):' ');var avatar=(avatar_size?('<a class="tweet_avatar" target=\"_blank\" href="'+user_url+'"><img src="'+avatar_url+'" height="'+avatar_size+'" width="'+avatar_size+'" alt="'+screen_name+'\'s avatar" title="'+screen_name+'\'s avatar" border="0"/></a>'):'');var time='<span class="tweet_time"><a target=\"_blank\" href="'+tweet_url+'" title="view tweet on twitter">'+tweet_relative_time+'</a></span>';var text='<span class="tweet_text">'+$([tweet_text]).makeHeart().capAwesome().capEpic()[0]+'</span>';return{item:item,screen_name:screen_name,user_url:user_url,avatar_size:avatar_size,avatar_url:avatar_url,source:source,tweet_url:tweet_url,tweet_time:tweet_time,tweet_relative_time:tweet_relative_time,tweet_raw_text:tweet_raw_text,tweet_text:tweet_text,user:user,join:join,avatar:avatar,time:time,text:text};});tweets=$.grep(tweets,s.filter).slice(0,s.count);list.append($.map(tweets.sort(s.comparator),function(t){return"<li>"+s.template(t)+"</li>";}).join('')).children('li:first').addClass('tweet_first').end().children('li:odd').addClass('tweet_even').end().children('li:even').addClass('tweet_odd');if(s.outro_text)list.after(outro);$(widget).trigger("loaded").trigger((tweets.length===0?"empty":"full"));if(s.refresh_interval){window.setTimeout(function(){$(widget).trigger("load");},1000*s.refresh_interval);}});}).trigger("load");});};})(jQuery);
/*
 ...
 * http://benalman.com/about/license/
 */
(function($,i,b){var j,k=$.event.special,c="location",d="hashchange",l="href",f=$.browser,g=document.documentMode,h=f.msie&&(g===b||g<8),e="on"+d in i&&!h;function a(m){m=m||i[c][l];return m.replace(/^[^#]*#?(.*)$/,"$1")}$[d+"Delay"]=100;k[d]=$.extend(k[d],{setup:function(){if(e){return false}$(j.start)},teardown:function(){if(e){return false}$(j.stop)}});j=(function(){var m={},r,n,o,q;function p(){o=q=function(s){return s};if(h){n=$('<iframe src="javascript:0"/>').hide().insertAfter("body")[0].contentWindow;q=function(){return a(n.document[c][l])};o=function(u,s){if(u!==s){var t=n.document;t.open().close();t[c].hash="#"+u}};o(a())}}m.start=function(){if(r){return}var t=a();o||p();(function s(){var v=a(),u=q(t);if(v!==t){o(t=v,u);$(i).trigger(d)}else{if(u!==t){i[c][l]=i[c][l].replace(/#.*/,"")+"#"+u}}r=setTimeout(s,$[d+"Delay"])})()};m.stop=function(){if(!n){r&&clearTimeout(r);r=0}};return m})()})(jQuery,this);// http://bit.ly/ishiv | WTFPL License
window.innerShiv=function(){function h(c,e,b){return/^(?:area|br|col|embed|hr|img|input|link|meta|param)$/i.test(b)?c:e+"></"+b+">"}var c,e=document,j,g="abbr article aside audio canvas datalist details figcaption figure footer header hgroup mark meter nav output progress section summary time video".split(" ");return function(d,i){if(!c&&(c=e.createElement("div"),c.innerHTML="<nav></nav>",j=c.childNodes.length!==1)){for(var b=e.createDocumentFragment(),f=g.length;f--;)b.createElement(g[f]);b.appendChild(c)}d=d.replace(/^\s\s*/,"").replace(/\s\s*$/,"").replace(/<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi,"").replace(/(<([\w:]+)[^>]*?)\/>/g,h);c.innerHTML=(b=d.match(/^<(tbody|tr|td|col|colgroup|thead|tfoot)/i))?"<table>"+d+"</table>":d;b=b?c.getElementsByTagName(b[1])[0].parentNode:c;if(i===!1)return b.childNodes;for(var f=e.createDocumentFragment(),k=b.childNodes.length;k--;)f.appendChild(b.firstChild);return f}}();

/*
function makeVideoMarkup(height, width, source, poster){
	var output = '';
	output+='<div id="silverlightControlHost">';
	output+='<object data="data:application/x-silverlight-2," type="application/x-silverlight-2" width="'+width+'" height="'+height+'">';
	output+='<param name="source" value="video/VideoPlayerM.xap"/>';
	output+='<param name="background" value="white" />';
	output+='<param name="initParams" value="m='+source+',autostart=true,thumbnail='+poster+'" />';
...
/*Silverlight video construction Function
function makeVideoMarkup(height, width, source, poster){
	var output = '<div id="silverlightControlHost">';
	output+='<object data="data:application/x-silverlight," type="application/x-silverlight" width="'+width+'" height="'+height+'">';
	output+='<param name="source" value="video/MediaPlayerTemplate.xap"/>';
	output+='<param name="onerror" value="onSilverlightError" />';
	output+='<param name="autoUpgrade" value="true" />';
	output+='<param name="minRuntimeVersion" value="4.0.50401.0" />';
	output+='<param name="enableHtmlAccess" value="true" />';
	output+='<param name="enableGPUAcceleration" value="true" />';
	output+='<param name="initparams" value="playerSettings = ';
	output+='<Playlist>';
	output+='<AutoLoad>true</AutoLoad>';
	output+='<AutoPlay>false</AutoPlay>';
	output+='<DisplayTimeCode>false</DisplayTimeCode>';
	output+='<EnableOffline>false</EnableOffline>';
	output+='<EnablePopOut>true</EnablePopOut>';
	output+='<EnableCaptions>true</EnableCaptions>';
	output+='<EnableCachedComposition>true</EnableCachedComposition>';
	output+='<StretchNonSquarePixels>StretchToFill</StretchNonSquarePixels>';
	output+='<StartMuted>true</StartMuted>';
	output+='<StartWithPlaylistShowing>false</StartWithPlaylistShowing>';
	output+='<Items>';
	output+='<PlaylistItem>';
	output+='<AudioCodec>WmaProfessional</AudioCodec>';
	output+='<Description></Description>';
	output+='<FileSize>8453857</FileSize>';
	output+='<IsAdaptiveStreaming>false</IsAdaptiveStreaming>';
	output+='<MediaSource>'+source+'</MediaSource>';
	output+='<ThumbSource>'+poster+'</ThumbSource>';
	output+='<Title>SuperSpeedwayFull</Title>';
	output+='<DRM>false</DRM>';
	output+='<VideoCodec>VC1</VideoCodec>';
	output+='<FrameRate>23.9760431376968</FrameRate>';
	output+='<Width>512</Width>';
	output+='<Height>288</Height>';
	output+='<AspectRatioWidth>16</AspectRatioWidth>';
	output+='<AspectRatioHeight>9</AspectRatioHeight>';
	output+='</PlaylistItem>';
	output+='</Items>';
	output+='</Playlist>"/> ';      
	output+='<div onmouseover="highlightDownloadArea(true)" onmouseout="highlightDownloadArea(false)">';
	output+='<img src="'+poster+'" style="position:absolute;width:'+width+';height:'+height+';border-style:none;" onerror="this.style.display="none"/>';
	output+='</div>';             
	output+='</object>';
	output+='<iframe id="_sl_historyFrame" style="visibility:hidden;height:0;width:0;border:0px"></iframe>';
	output+='</div>';
